...
type Tab = "dashboard" | "active-orders" | "all-orders" | "users" | "payments" | "disputes";
...
export default function ClientAdminPanel() {
  ...
  const fetchAll = async () => {
    try {
      const [statsRes, ordersRes, usersRes, settingsRes, disputesRes, depositsRes] = await Promise.all([
      ...
      const [statsData, ordersData, usersData, settingsData, disputesData, depositsData] = await Promise.all([
        statsRes.json(), ordersRes.json(), usersRes.json(), settingsRes.json(), disputesRes.json(), depositsRes.json()
      ]);
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "var(--bg-secondary)" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <button onClick={() => { fetch("/api/auth/logout", { method: "POST" }); router.push("/"); }} className="btn btn-ghost btn-sm">Log Out</button>
        </div>
      </header>
                  ...
                  <tbody>
                    {orders.map(o => (
                      <tr key={o.id}>
                        ...
                        <td>
                          {o.orderSource === "TELEGRAM" ? "📱 Telegram" : "🌐 Website"}
                        </td>
                        <td><span className={`badge ${o.status === "COMPLETED" ? "badge-green" : o.status === "READY" ? "badge-blue" : o.status === "COOLDOWN_ACTIVE" ? "badge-orange" : "badge-in_stock"}`}>{o.status}</span></td>
                  ...
                  <tbody>
                    {users.map(u => (
                      <tr key={u.id}>
                        ...
                        <td style={{ fontWeight: "600", color: "var(--green)" }}>${u.wallet.balance.toFixed(2)}</td>
                        <td>{u.totalOrders}</td>
                        <td style={{ fontWeight: "600" }}>${u.totalSpent.toFixed(2)}</td>
          ...
          {activeTab === "payments" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "24px", animation: "fadeIn 0.4s ease" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", alignItems: "start" }}>
                <div className="card">
                  <h3 style={{ marginBottom: "16px", fontSize: "18px" }}>Crypto Wallet Address</h3>
                  <p style={{ color: "var(--text-secondary)", fontSize: "14px", marginBottom: "24px" }}>
                    This is the wallet address displayed to customers when they attempt to deposit funds into their account via the website dashboard.
                  </p>
                ...
                <div className="card">
                  <h3 style={{ marginBottom: "16px", fontSize: "18px" }}>Info & Warnings</h3>
                  <p style={{ color: "var(--text-secondary)", fontSize: "14px", lineHeight: "1.6" }}>
                    🔒 <strong>Secure Ledger System:</strong> Users request a deposit, transfer crypto, and their wallet balance is updated <strong>ONLY</strong> when you manually approve it here.
                  </p>
                  <p style={{ color: "var(--text-secondary)", fontSize: "14px", lineHeight: "1.6", marginTop: "12px" }}>
                    ⚠️ Verify that you have actually received the specified amount in your wallet address before clicking <strong>Approve</strong>.
                  </p>
                </div>
              </div>
              ...
              <div className="card">
                <h3 style={{ marginBottom: "16px", fontSize: "18px" }}>Deposit Requests Review</h3>
                {deposits.length === 0 ? (
